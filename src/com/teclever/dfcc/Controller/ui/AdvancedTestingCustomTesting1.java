...
import com.teclever.resultstore.entities.Person;
...
public class AdvancedTestingCustomTesting1 {
    ...
    private ObservableList<String> symbolNameList = FXCollections.observableArrayList();
    
    ListView<SymbolDto> listView = new ListView<>();
    ...
    private ListView<String> symbolListView = new ListView<>();
    ...
    private GridPane createLeftTerminalBox() {
        ...
        symbolTextField.setOnKeyReleased(this::handleSymbolTextFieldKeyReleased);
        ...
        symbolListView.setVisible(false);
        symbolListView.setOnMouseClicked(event -> handleSymbolListViewSelection());

        VBox symbolTextFieldVBox = new VBox(symbolTextField, symbolListView);
        terminalCommandGridPane.add(symbolTextFieldVBox, 1, 0);
    ...
    private void handleSymbolTextFieldKeyReleased(KeyEvent event) {
        String input = symbolTextField.getText().toLowerCase();
        if (input.isEmpty()) {
            symbolListView.setVisible(false);
        } else {
            List<SymbolDto> filteredList = new ArrayList<SymbolDto>();
            		
            symbolDataList.stream()
                    .filter(symbol -> symbol.getSymbolName().toLowerCase().contains(input))
                    .collect(Collectors.toList());

            ObservableList<String> filteredNames = FXCollections.observableArrayList(
                    filteredList.stream()
                            .map(SymbolDto::getSymbolName)
                            .collect(Collectors.toList())
            );

            symbolListView.setItems(filteredNames);
            symbolListView.setVisible(!filteredNames.isEmpty());
        }
    }

    private void handleSymbolListViewSelection() {
        String selectedName = symbolListView.getSelectionModel().getSelectedItem();
        if (selectedName != null) {
            symbolTextField.setText(selectedName);
            SymbolDto selectedTestType = symbolDataList.stream()
                    .filter(testType -> testType.getSymbolName().equals(selectedName))
                    .findFirst()
                    .orElse(null);

            if (selectedTestType != null) {
                typeTextField.setText(selectedTestType.getSymbolType());
                minValueTextField.setText(selectedTestType.getMin());
                maxValueTextField.setText(selectedTestType.getMax());
                
                ipDataTextField.setText("");
            }
            symbolListView.setVisible(false);
        }
    }
    ...
    private void initializeTestTypeComboBox() {
        ...
        AdvanceCustom1TestingManagement advanceCustom1TestingManagement = new AdvanceCustom1TestingManagement();
   
        //For 
        symbolDataList =   FXCollections.observableArrayList(advanceCustom1TestingManagement.getAllSymbolsForAdavanceTest("UUT2","TT5"));
        for (SymbolDto testType : symbolDataList) {
        	symbolNameList.add(testType.getSymbolName());
        }
       
    }
    
    
    	symbolTextField.setOnKeyReleased(event -> {
            String text = symbolTextField.getText().toLowerCase();
            if (text.isEmpty()) {
               listView .setVisible(false);
            } else {
                List<SymbolDto> filteredList = symbolDataList.stream()
                    .filter(person -> person.getSymbolName().toLowerCase().contains(text))
                    .collect(Collectors.toList());

                listView.setItems(FXCollections.observableArrayList(filteredList));
                listView.setVisible(!filteredList.isEmpty());
            }
        })

        listView.setOnMouseClicked(event -> {
            SymbolDto selectedsymbol = listView.getSelectionModel().getSelectedItem();
            if (selectedsymbol != null) {
                symbolTextField.setText(selectedsymbol.getSymbolName());
                //idLabel.setText("ID: " + selectedPerson.getId());
                //addressLabel.setText("Address: " + selectedPerson.getAddress());
                //phoneLabel.setText("Phone: " + selectedPerson.getPhoneNumber());
                listView.setVisible(false);
            }
        })
