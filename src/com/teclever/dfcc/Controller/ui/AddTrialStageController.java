...
public class AddTrialStageController implements Initializable {
	@FXML
	...
	private TrialsConfigurationController mainPageController;

	public void setMainPageController(TrialsConfigurationController mainPageController) {
	...
	void onClickAddOrEditStage1(ActionEvent event) {
		...
		if (add_new_stage_button.getText().equals("Add New Stage")) {
			LevelOneAddResponse response = stageConfig.addLevelOneStageMaster(stage_name_field.getText(),
					"ST4", UUT_ID,  mandatoryCheckBox.isSelected(), continueWithErrorCheckBox.isSelected());
			...
			LevelOneAddResponse response = stageConfig.updateLevelOneStageMaster(sessionStage.getId(),
					stage_name_field.getText().trim(), "ST4", UUT_ID,   mandatoryCheckBox.isSelected(), continueWithErrorCheckBox.isSelected());
			if (response.getResponse().getResponseCode() == 1) {
				closeAndRefresh();
			} else {
				Notifications.showErrorAlert(response.getResponse().getResponseMessage());
	...
	private void closeAndRefresh() {
		Stage stage = (Stage) addStageMainContainer.getScene().getWindow();
		stage.close();
		mainPageController.refreshTrialsList();
	}
